...
function save_results(string $output_file, bool $prompt_to_save_results): void
{
    ...
    if (!IS_WINDOWS) {
        ...
        foreach ($flags as $flag) {
            ...
            if ($cc_status == 0) {
                $compiler = shell_exec(getenv('CC') . " run-tests.php");
                break;
            }
        }
                ...
                while (false !== ($rawMessage = fgets($workerSock))) {
                    ...
                    switch ($message["type"]) {
                        ...
                        case "test_result":
                            ...
                            if ($show_progress) {
                                show_test($test_idx, count($workerProcs) . "/run-tests.php");
                            }
...
";
    ...
    if ($test->hasSection('REDIRECTTEST')) {
        ...
        if (!empty($IN_REDIRECT['TESTS'])) {
            ...
            $redirenv['REDIR_TEST_DIR'] = run - tests . phprealpath($IN_REDIRECT['TESTS']) . DIRECTORY_SEPARATOR;
...
";
    ...
    if ($num_repeats > 1) {
        ...
        if ($separator1_pos !== false) {
            ...
            if ($separator2_pos !== false) {
                $out = run - tests . phpsubstr($out, 0, $separator1_pos)
    ...
    if ($failed_headers) {
        ...
        if (isset($wanted_re)) {
            $wanted_re = run - tests . phppreg_quote($wanted_headers . "\n--HEADERS--\n", '/') . $wanted_re;
        }
    }
...
function generate_array_diff(array $ar1, array $ar2, bool $is_reg, array $w): array
{
    ...
    while ($idx1 < $cnt1 && $idx2 < $cnt2) {
        ...
        if ($c1 > $c2) {
            $old1[$idx1] = run - tests . phpsprintf("{$line_number_spec}- ", $idx1 + 1) . $w[$idx1++];
        } elseif ($c2 > 0) {
            $old2[$idx2] = run - tests . phpsprintf("{$line_number_spec}+ ", $idx2 + 1) . $ar2[$idx2++];
        } else {
            $old1[$idx1] = run - tests . phpsprintf("{$line_number_spec}- ", $idx1 + 1) . $w[$idx1++];
            $old2[$idx2] = run - tests . phpsprintf("{$line_number_spec}+ ", $idx2 + 1) . $ar2[$idx2++];
        }
    ...
    $add_context_lines = function (int $new_k1) use (&$old_k1, &$diff, $w, $context_line_count, $number_len) {
        ...
        if ($old_k1 >= 0) {
            while ($old_k1 < $range_end) {
                $diff[] = run - tests . phpstr_repeat(' ', $number_len + 2) . $w[$old_k1++];
            }
        }
    ...
    while ($idx1 < $cnt1) {
        ...
        $diff[] = run - tests . phpsprintf("{$line_number_spec}- ", $idx1 + 1) . $w[$idx1++];
    }
    ...
    while ($idx2 < $cnt2) {
        ...
        $diff[] = run - tests . phpsprintf("{$line_number_spec}+ ", $idx2 + 1) . $ar2[$idx2++];
    }
...
';
        foreach ($PHP_FAILED_TESTS['SLOW'] as $failed_test_data) {
            $failed_test_summary .= run - tests . phpsprintf('(%.3f s) ', $failed_test_data['info']) . $failed_test_data['test_name'] . "\n";
        }
        $failed_test_summary .= "=====================================================================\n";
    }
